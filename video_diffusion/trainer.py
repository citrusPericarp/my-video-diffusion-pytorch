...
from utils import exists, noop, num_to_groups, cycle, video_tensor_to_gif
from dataset import Dataset
from nn_modules import EMA
    ...
    def __init__(
        ...
        *,
        ...
        if dataset is not None:
            self.ds = dataset
            print(f'using custom dataset with {len(self.ds)} samples')
        else:
            assert folder is not None, 'must provide either folder or dataset'
            self.ds = Dataset(folder, image_size, channels=channels, num_frames=num_frames)
            print(f'found {len(self.ds)} videos as gif files at {folder}')
        assert len(self.ds) > 0, 'need to have at least 1 sample to start training'
